/**
 * Client-side Claude API service for text generation
 * Makes direct API calls to Claude from the browser
 */
 ...
 */
export async function generateClientSummary(pdfData, apiKey) {
    if (!apiKey) {
...
function createSummaryPrompt(pdfData) {
    ...
    const tablesInfo = pdfData.tables.length > 0
        ? `The document contains ${pdfData.tables.length} tables. Here's the first one: ${JSON.stringify(pdfData.tables[0].rows?.slice(0, 5) || pdfData.tables[0].slice(0, 5))}`
 ...
 */
export function generateLocalSummary(pdfData) {
